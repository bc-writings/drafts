...
var grvt_wrapper = {
    ...
    triggerSetUserConsentEvent: function(token, processing_token, tcData,check_also_mr) {
        ...
        var grvt_has_consent = grvt_wrapper.cmplist_tcData_to_consent(tcData, cmp_list_v2,check_also_mr); 
        if(grvt_has_consent){
    ...
    cmplist_tcData_to_consent: function (tcData, cmp_list_v2,check_also_mr) {
        ...
        if(check_also_mr){
            ...
            purpconsents = purpconsents && tcData.vendor.consents[501] && tcData.vendor.consents[184] && cmp_is_valid;
        }
        else {
            if(debugMode){console.log("[Gravity][TCFV2] Consent Summary \n CMP : " + cmp_desc + "\n purpose 1 : " + pconsents[1] + "\n purpose 2 : " + pconsents[2] + "\n purpose 3 : " + pconsents[3] + "\n purpose 4 : " + pconsents[4] + "\n purpose 7 : " + pconsents[7] + "\n purpose 9 : " + pconsents[9] + "\n vendor 501 : " + tcData.vendor.consents[501]);}
    ...
    checkGravityConsents: function(token, processing_token, cb,check_also_mr = false){
        ...
        if (typeof window.__tcfapi !== "undefined") {
            ...
            __tcfapi('addEventListener', 2, (tcData, success) => {
                ...
                if(success && (tcData.eventStatus == "tcloaded" || tcData.eventStatus == "useractioncomplete") && (tcData.cmpId != 7 || Object.keys(tcData.purpose.consents).length != 0)) {
                    ...
                    window.__tcfapi('getTCData', 2, (tcData, success) => {
                        if((success != false) && tcData.cmpStatus == "loaded") {   
                            ...
                            if(debugMode){console.log("[Gravity][TCFV2] Unexpected Error : Loaded CMP did not managed to pull TC String");}
                            if (grvt_tries >= 10) {
                                if(debugMode){console.log("[Gravity][TCFV2] Fatal Error : CMP did not managed to pull TC String after several retries => no consent");}
                                cb(false);
                            } else {
                                grvt_tries += 1;
            ...
            window.__cmp('getVendorConsents', [501,184], function(result) {
