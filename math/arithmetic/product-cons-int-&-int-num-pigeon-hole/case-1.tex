...
\begin{remark}
	La fonction \piton{sol} du programme précédent traduit l'algorithme suivant que nous utiliserons dans la suite de ce document.
\end{remark}
...
%	\caption{Classique et efficace} 
    ...
    \Actions{
		...
		\\
		\ForRange{N}{1}{1 + \quot{\delta + 1}{2}}{	
			$M \Store N^2 - \delta$
			\\
			\If{$M \geq 0$}{
				$M \Store \floor{\sqrt{M}}$
				\\
				\If{$M \neq 0$ \And $M^2 = N^2 - \delta$}{
