...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Ici, $n + i = ad A^2$ et $n + i + d = ad B^2$ donnent $ad (B^2 - A^2) = d$\,, puis $a (B^2 - A^2) = 1$ qui a été rejeté dans l'explication précédente.

		\item Ici, $n + i = ad A^2$ et $n + i + 2 d = ad B^2$ donnent $ad (B^2 - A^2) = 2d$\,, \emph{i.e.} $a (B^2 - A^2) = 2$\,, d'où $B^2 - A^2 \in \setgene{1, 2}$ qui est impossible d'après l'explication \ref{vogler-no-witness} ci-dessus et le fait \ref{dist-square}.
	\end{enumerate}
\end{proof}
...
\begin{example}
	La multiplication de deux $d$-tableaux de Vogler est \enquote{naturelle} lorsqu'elle porte sur des $d$ premiers entre eux.
...
\begin{fact} \label{vogler-parity-square}
	$\forall p \in \PP$\,, si $\consprod\ \in \NNsquare$\,, alors dans le $p$-tableau de Vogler associé à $\consprod\ \in \NNsquare$\,, le nombre de valeurs $p$ est forcément pair.
\end{fact}
