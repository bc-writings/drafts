...
\begin{fact} \label{sftable-sub-square}
...
\begin{fact} \label{sftable-illegal}
	Soit $(n, d, a) \in ( \NNs )^3$ et $i \in \NN$\,.
	Les \sftab[x] ci-après sont impossibles
	(de nouveau, les puces $\bullet$ indiquent des valeurs quelconques).
...
\begin{defi}
	...
	Cette situation est résumée par le tableau suivant qui sera nommé \enquote{\sftab\ partiel}\,
	\footnote{
...
\begin{example}
	...
	\vspace{-1ex}
	La multiplication de ces \sftab[x] partiels est le \sftab\  suivant, partiel a priori.
	Si de plus, $2$ et $3$ sont les seuls diviseurs premiers de $\consprod<3>$\,, alors on obtient un \sftab\ non partiel.
...
\begin{fact} \label{sftable-multiple}
	Dans la deuxième ligne d'un \sftab\ partiel relatif à un unique nombre premier $p$\,, les positions des valeurs $p$ sont congrus modulo $p$, et les autres valeurs ne sont que des $1$\,.
\end{fact}
...
\begin{fact} \label{sftable-parity-square}
	...
	alors dans le \sftab\ partiel relatif associé à $\consprod$\,, et uniquement au nombre premier $p$\,, le nombre de valeurs $p$ est forcément pair.
\end{fact}
